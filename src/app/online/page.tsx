...
export default function OnlinePage() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-24 pt-20">
      ...
      <div className="mb-8 p-4 border rounded-lg text-center w-full max-w-sm">
        <p>サーバーとの接続状態: <span className="font-bold">{status}</span></p>
        {roomId && <p className="mt-2">ルームID: <span className="font-bold text-blue-500">{roomId}</span></p>}
      ...
      {!roomId && (
        <>
          <div className="w-full max-w-sm p-8 border rounded-lg shadow-lg">
            <h2 className="text-2xl font-bold mb-6 text-center">ルームを作成する</h2>
            <button onClick={handleCreateRoom} className="w-full bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
              ルームを作成
            </button>
          </div>

          <div className="my-8 text-center text-xl font-bold">OR</div>

          <div className="w-full max-w-sm p-8 border rounded-lg shadow-lg">
            <h2 className="text-2xl font-bold mb-4 text-center">ルームに参加する</h2>
            <div className="flex flex-col gap-4">
              ...
              />
              <button onClick={handleJoinRoom} className="w-full bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                ルームに参加
              </button>
            </div>
          </div>
